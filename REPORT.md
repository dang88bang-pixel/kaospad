...
- [x] **Phase 4 — Funktionstest:** `make test` **exit 0** lokal, **34 Ergebniszeilen** (11 neue Suiten: resilience, watchdog, log-rotation, bug-report, engine-service, flatbuffers-pcm, dsp-error-handling, failure-simulation, dex-builder, real-asr, wasm-cross-runtime). CI-Run `34722066403`: **4/4 Jobs grün**, Playwright **7 von 7 Specs** im echten Chromium (1 Spec-Datei `tests/ui/*.spec.mjs` mit 7 `test(`-Einträgen, lokal gezählt).
...
`make test` → **exit 0** (letzter lokaler Lauf, 34 Ergebniszeilen). Auszug:
...
dex builder verified: DEX-035 echt und parsebar (504 Bytes, Adler-32 + SHA-1 korrekt, const/4+return und invoke-direct+return-void)
real asr verified: 59 checks // Vorverarbeitung exakt // Standardweg deterministisch // echter Dekodierlauf ausgeführt
wasm cross-runtime verified: 177 checks // wasmtime == Node/V8 == Python // FNV-1a bitgleich über dasselbe Modul
...
Belege, HTTP-Codes und Aufwand je Blocker: **`docs/audit/ONLINE-ALTERNATIVEN.md`** (Evaluation vom 2026-09-12; 4 von 8 Blockern real beseitigt bzw. real umgesetzt).
...
4. ~~**Echte Modellgewichte**~~ — **teilweise behoben (Online-Alternative):** Whisper/MiDaS bleiben unerreichbar (HuggingFace `000`, GitHub-Release-Assets `302 → 0 Bytes`, weil `objects.githubusercontent.com` gesperrt ist), aber **`pocketsphinx 5.1.1`** kommt per PyPI und enthält ein vollständiges akustisches Modell (`mdef`, `means`, `variances`, `sendump`, `transition_matrices`, LMs, CMU-Wörterbuch). `engines/whisper_offline/real_asr.py` dekodiert damit **echt und vollständig offline**; aktivierbar über `tflite_runtime.infer(..., real_asr_enabled=True)`. **Opt-in**, weil echte Dekodierung die deterministische SHA-256-Parität aufhebt; echte Ergebnisse sind gekennzeichnet (`engine: pocketsphinx`, `real_weights: true`, `deterministic: false`). Ohne pocketsphinx kommt `RealAsrUnavailable` statt stillem Rückfall. Test: `real asr verified: 59 checks`. **Grenze:** Englisch statt Deutsch, anderes Modell als Whisper.
...
6. **Emscripten (`emcc`)** für `web/wasm/dsp_core.mjs`: `emsdk` liegt zwar auf GitHub, lädt seinen Clang aber von `storage.googleapis.com` (gesperrt) → nicht installierbar. **Workaround:** ABI-Modul `dist/wasm/kaoss_dsp.wasm` wird mit `ziglang` gebaut und ist paritätsgeprüft; UI nutzt sonst den JS-Kern. **Neu abgesichert:** `engines/wasm_cross_runtime.py` führt dasselbe Modul zusätzlich in **wasmtime 48.0.0** (PyPI ✅) aus — zwei unabhängige Laufzeiten, **FNV-1a über die Float-Bits bitgleich zu Node/V8**, max. Abweichung `4.30e-08`. Test: `wasm cross-runtime verified: 177 checks`.
